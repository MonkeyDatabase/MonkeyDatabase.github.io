...
| Element ID | Length | Current Operating<br/>Classes | Operating<br/>Classes | Current Operating Classes Extension Sequence<br/>(optional) | Operating Classes Duple Sequence<br/>(optional) |
| :--------: | :----: | :---------------------------: | :-------------------: | :---------------------------------------------------------: | :---------------------------------------------: |
...
1. 既然会依标准的规定传送Beacon帧，但不包含SSID信息元素，这样当时的用户端软件就不会显示该网络。
2. 要与之连接，工作站必须送出包含SSID的Probe Request帧，因此它的作用类似一把隐藏的密钥，使用隐藏SSID功能的基站只接受在Probe Request中给之遭隐藏的SSID的工作站。目前即便该STA发送Probe request包SSID是正确的，在AP回复STA时Probe response包中也不会带上SSID，做到ssid的隐藏。
